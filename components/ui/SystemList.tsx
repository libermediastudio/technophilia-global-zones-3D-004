...
export const SystemList: React.FC<SystemListProps> = ({ bodies, onSelect }) => {
  ...
  // No exclusions - show everything in the SOL system
    ...
    <div className="absolute top-24 right-6 w-64 pointer-events-none z-30 font-mono hidden md:block">
       <div className="bg-[#121212]/80 backdrop-blur-sm p-2 pointer-events-auto border-b border-[#E42737]/50 mb-2 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
           ...
           <div className="relative">
                <input type="text" placeholder="SEARCH SECTOR..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
       ...
       <div className="max-h-[40vh] overflow-y-auto pr-1 space-y-1 bg-[#121212]/80 backdrop-blur-sm p-2 border-b border-[#E42737]/50 shadow-[0_0_20px_rgba(0,0,0,0.5)] pointer-events-auto [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-[#121212] [&::-webkit-scrollbar-thumb]:bg-[#E42737]">
            ...
            return (
              <button key={body.id} onClick={() => onSelect(body.id)}
                ...
                <div className="flex justify-between items-center">
                  <span className="text-xs tracking-wider text-slate-300 group-hover:text-white font-bold transition-colors">{body.name}</span>
          ...
          {filteredBodies.length === 0 && <div className="p-4 text-center text-[10px] text-slate-500 italic border-l-2 border-slate-800 bg-[#121212]/60">NO SECTORS FOUND</div>}
