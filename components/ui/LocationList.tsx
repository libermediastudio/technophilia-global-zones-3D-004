    ...
    <div className="absolute top-24 right-6 w-64 pointer-events-none z-30 font-mono hidden md:block">
       <div className="bg-[#121212]/80 backdrop-blur-sm p-2 pointer-events-auto border-b border-[#E42737]/50 mb-2 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
           ...
           <div className="relative">
                <input type="text" placeholder="SEARCH TARGET..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
          ...
          {filteredCities.map((city) => {
            ...
            return (
              <button key={city.name} onClick={() => onSelect(city)}
                className={`w-full text-left p-2 border-l-2 transition-all duration-300 relative group ${isSelected ? 'bg-red-900/30 border-[#E42737] pl-4' : 'bg-[#121212]/60 border-transparent hover:border-[#E42737]/50 hover:bg-red-900/10 hover:pl-3'}`}>
                <div className="flex justify-between items-center">
                  <span className={`text-xs tracking-wider ${isSelected ? 'text-white font-bold' : 'text-slate-400 group-hover:text-red-200'}`}>{city.name}</span>
                  {isSelected && <MapPin size={12} className="text-[#E42737]" />}
                </div>
                {isSelected && <div className="absolute top-0 right-0 w-1 h-1 bg-[#E42737]"></div>}
                <div className="flex justify-between mt-1 opacity-60">
                    <span className="text-[8px] text-[#E42737]">{city.category}</span>
                    {city.faction && city.faction !== city.category && <span className="text-[8px] text-slate-500">{city.faction}</span>}
                </div>
              </button>
            );
          })}
          {filteredCities.length === 0 && <div className="p-4 text-center text-[10px] text-slate-500 italic border-l-2 border-slate-800 bg-[#121212]/60">NO TARGETS FOUND</div>}
          <div className="h-1 w-full bg-[#E42737]/20 mt-2 flex justify-end"><div className="w-1/3 h-full bg-[#E42737]/50"></div></div>
       </div>
    </div>
  );
};
